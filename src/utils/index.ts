...
export const generateRandomNote = (): Promise<Note> => {
  ...
  const randomCurrency: string =
    currencies[Math.floor(Math.random() * currencies.length)];
  const numberOfCounts: number = Math.floor(Math.random() * 5) + 1; // Adjust the range as needed

  // Generate an array of counts with random values
  const counts = Array.from({ length: numberOfCounts }, () => {
    const randomDenomination: number =
  ...
  // Calculate total_amount based on the sum of counts
  const totalAmount = counts.reduce(
    (acc, curr) => acc + curr.count * curr.currency_note,
    0
  );
